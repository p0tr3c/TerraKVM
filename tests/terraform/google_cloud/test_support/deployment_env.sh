...
export GCLOUD_KEYFILE_JSON=`aws ssm get-parameters --name /terrakvm/gcloud_access_file --region "$SSM_REGION" | jq -r '.Parameters[0].Value'`
...
export TF_VAR_gcloud_zone=`aws ssm get-parameters --name /terrakvm/gcloud_zone --region "$SSM_REGION" | jq -r '.Parameters[0].Value'`
